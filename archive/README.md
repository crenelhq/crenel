...
> Entries marked *(private repo)* are kept out of the public snapshot
> (`.gitattributes` `export-ignore`) — process/diagnostic fat, not proof material.
> The trial plans and results below ARE exported.

- **`incident-2026-06-27/`** *(private repo)* — the 2026-06-27 Caddy-wedge postmortem +
  goroutine-dump root-cause diagnostics. The bug is fixed (shipped in v0.2.0); kept in
  case the failure mode ever recurs.
...
- **`outputs/`** *(private repo)* — a pre-launch checklist snapshot, superseded by
  `STATE-OF-CRENEL.md`'s own backlog section.
- **`diag-artifacts/`** *(private repo)* — raw goroutine/log captures from the
  2026-06-27 incident, companion to `incident-2026-06-27/`.
- **`BUILD_LOG.md`** *(private repo)* — the frozen per-increment development narrative
  (M0 → v0.3.1). Release-level history continues in the root
  [`CHANGELOG.md`](../CHANGELOG.md).
- **`CONTEXT.md`** *(private repo)* — historical maintainer-continuity notes
  (conventions, roadmap snapshots) from the pre-launch build.
- **`PROVING-GROUND.md`** *(private repo)* — the maintainer's standing live bench (real
  Caddy / Traefik / nginx / Authentik) used for the trial-before-merge cadence described
